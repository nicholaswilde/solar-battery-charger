...
Instead of specifying `-b` and `-p` for every command, a [sketch.json][4] may be stored in the sketch directory
that sets the defaults for that sketch.

The `sketch.json` file may be generated by using the [arduino-cli board attach][5] command.
